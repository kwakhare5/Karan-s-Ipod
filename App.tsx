...
const App = () => {
  ...
  const { chassisColor, setChassisColor, clockSettings, setClockSettings, favorites, toggleFavorite } = useSettings();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { fetchLibrary(); }, 0);
    return () => clearTimeout(t);
  }, [fetchLibrary]);

  const renamePlaylist = useCallback(async (id: string, oldName: string) => {
  ...
  // -- Menu Logic --
  const executeItemAction = useCallback((item: MenuItem) => {
    ...
    if (item.type === 'navigation' && item.targetMenuId) {
      ...
      if (item.id === 'show_seconds') setClockSettings(s => ({ ...s, showSeconds: !s.showSeconds }));
      if (item.id === 'date_format') setClockSettings(s => ({ ...s, isLongDate: !s.isLongDate }));
  ...
  // -- Music Handlers --
  const handleSearchSelect = useCallback((track: Track, results: Track[]) => {
    const index = results.findIndex(t => t.videoId === track.videoId);
    music.setShuffle(false);
    music.play(track, results, index >= 0 ? index : 0);
    setGlobalSearchResults([]); // Clear results on navigate
    navigateTo(MenuIDs.NOW_PLAYING);
  }, [music, navigateTo]);

  const handlePlaylistSearchSelect = useCallback(async (track: Track) => {
  ...
  const handleNext = useCallback(() => { music.next(); }, [music]);
  const handlePrev = useCallback(() => { music.prev(); }, [music]);
  ...
  const currentMenuItems = useMemo<MenuItem[]>(() => {
    ...
    if (menuId === MenuIDs.SETTINGS_CLOCK) {
      return ROOT_MENUS[menuId].map(item => {
        ...
        if (item.id === 'time_format') label = `Time Format: ${clockSettings.is24Hour ? '24-hour' : '12-hour'}`;
        if (item.id === 'show_seconds') label = `Show Seconds: ${clockSettings.showSeconds ? 'On' : 'Off'}`;
        if (item.id === 'date_format') label = `Date Format: ${clockSettings.isLongDate ? 'Long' : 'Short'}`;
        return { ...item, label };
      });
    }

    if (menuId === MenuIDs.SEARCH) {
      return globalSearchResults.map((track): MenuItem => ({
        id: track.videoId,
    ...
    if (menuId === MenuIDs.FAVORITES) {
      if (favorites.length === 0) return [{ id: 'no_favs', label: 'No Favorites Yet', type: 'toggle' as MenuItemType }];
      return favorites.map((fav): MenuItem => ({
        id: fav.id,
        ...
        action: () => {
          ...
          play(track, favorites.map(f => ({
            ...
            album: f.albumId
          })), favorites.findIndex(f => f.id === fav.id));
          navigateTo(MenuIDs.NOW_PLAYING);
        }
      }));
    }
    ...
    if (menuId === MenuIDs.ARTISTS) {
      if (sortedArtists.length === 0) return [{ id: 'no_artists', label: 'No Artists Found', type: 'toggle' as MenuItemType }];
      return sortedArtists.map((artist): MenuItem => ({
        id: artist.id,
        label: artist.name,
        type: 'navigation' as MenuItemType,
        targetMenuId: MenuIDs.ARTIST_DETAIL,
        hasChevron: true,
        action: () => { setSelectedArtistId(artist.id); navigateTo(MenuIDs.ARTIST_DETAIL); }
      }));
    }

    if (menuId === MenuIDs.ARTIST_DETAIL && selectedArtistId) {
      const targetArtistName = libraryArtists.find(a => a.id === selectedArtistId)?.name || selectedArtistId;
      const artistSongs = librarySongs
        .filter(s => s.artistId === selectedArtistId || s.artist === targetArtistName)
        .sort((a, b) => a.title.localeCompare(b.title));
      
      if (artistSongs.length === 0) return [{ id: 'no_songs', label: 'No songs found', type: 'toggle' }];
    ...
    if (menuId === MenuIDs.ALBUMS) {
      ...
      return albumsArray.map(([album, artist]) => ({
        ...
        hasChevron: true,
        action: () => { setSelectedAlbumId(album); navigateTo(MenuIDs.ALBUM_DETAIL); }
      }));
    }
    ...
    if (menuId === MenuIDs.ALBUM_DETAIL && selectedAlbumId) {
      const albumSongs = librarySongs
        ...
        .sort((a, b) => a.title.localeCompare(b.title));
      if (albumSongs.length === 0) return [{ id: 'no_songs', label: 'No songs found', type: 'toggle' }];
    ...
    if (menuId === MenuIDs.PLAYLISTS) {
      return [
        {
          id: 'create_playlist', label: 'Create New Playlist', type: 'action' as const, action: async () => {
        ...
        ...playlists.map((p): MenuItem => ({
          ...
          hasChevron: true,
          action: () => { setSelectedPlaylistId(p.id); navigateTo(MenuIDs.PLAYLIST_DETAIL); }
        }))
      ] as MenuItem[];
    }
    ...
    if (menuId === MenuIDs.PLAYLIST_DETAIL) {
      ...
      if (!playlist) return [];
      const playlistSongs = (playlist.songIds || []).map(sid => librarySongs.find(s => s.videoId === sid)).filter(Boolean) as Track[];
      return [
        {
          id: 'add_songs_search', label: '+ Add Songs', type: 'navigation' as const, targetMenuId: MenuIDs.PLAYLIST_SEARCH, action: () => {
            setAddingToPlaylistId(selectedPlaylistId);
            setPlaylistSearchQuery('');
            navigateTo(MenuIDs.PLAYLIST_SEARCH);
          }
        },
        ...playlistSongs.map((track, index): MenuItem => ({
          id: track.videoId,
          label: track.title,
          type: 'action' as const,
          action: () => {
            setShuffle(false);
            play(track, playlistSongs, index);
            navigateTo(MenuIDs.NOW_PLAYING);
          }
        })),
        { id: 'rename_playlist', label: 'Rename Playlist', type: 'action' as const, action: () => renamePlaylist(playlist.id, playlist.name) },
        { id: 'delete_playlist', label: 'Delete Playlist', type: 'action' as const, action: () => deletePlaylist(playlist.id) },
      ] as MenuItem[];
    }

    if (menuId === MenuIDs.PLAYLIST_SEARCH) {
      const results = librarySongs.filter(s =>
      ...
      return results.map((track): MenuItem => ({
        id: track.videoId,
    ...
    if (menuId === MenuIDs.GENRES) {
      const availableGenres = Array.from(new Set(librarySongs.map(s => s.genre).filter(Boolean))) as string[];
      if (availableGenres.length === 0) return [{ id: 'no_genres', label: 'No Genres Found', type: 'toggle' as MenuItemType }];
      return availableGenres.sort().map((g): MenuItem => ({
        id: g, label: g, type: 'navigation' as MenuItemType, targetMenuId: MenuIDs.GENRE_DETAIL, hasChevron: true,
        action: () => { setSelectedGenreId(g); navigateTo(MenuIDs.GENRE_DETAIL); } 
      }));
    }

    if (menuId === MenuIDs.GENRE_DETAIL && selectedGenreId) {
       const genreSongs = librarySongs.filter(s => s.genre === selectedGenreId).sort((a,b) => a.title.localeCompare(b.title));
       if (genreSongs.length === 0) return [{ id: 'no_songs', label: 'No songs found', type: 'toggle' as MenuItemType }];
       return genreSongs.map((song, idx): MenuItem => ({
         id: song.videoId, label: song.title, type: 'action' as MenuItemType,
         action: () => {
           setShuffle(false);
           play(song, genreSongs, idx);
           navigateTo(MenuIDs.NOW_PLAYING);
         }
       }));
    }
    ...
    if (menuId === MenuIDs.SONGS) {
      return sortedSongs.map((song, idx) => ({
        id: song.videoId, label: `${song.title} - ${song.artist}`, type: 'action',
        action: () => {
          setShuffle(false);
          play(song, sortedSongs, idx);
          navigateTo(MenuIDs.NOW_PLAYING);
        }
      }));
    }
    ...
    if (menuId === MenuIDs.CONTACTS) {
      const items: MenuItem[] = [{
        id: 'add_contact', label: 'Add Contact', type: 'navigation', targetMenuId: MenuIDs.CONTACT_ADD,
        action: () => { setContactForm({ firstName: '', lastName: '', phone: '', email: '' }); setIsEditingContact(false); setSelectedContact(null); navigateTo(MenuIDs.CONTACT_EDIT); }
      }];
      contacts.sort((a, b) => (a.firstName + a.lastName).localeCompare(b.firstName + b.lastName)).forEach(c => {
        items.push({
          id: c.id, label: `${c.firstName} ${c.lastName}`, type: 'navigation', targetMenuId: MenuIDs.CONTACT_DETAIL, hasChevron: true,
          action: () => { setSelectedContact(c.id); navigateTo(MenuIDs.CONTACT_DETAIL); }
        });
      });
      return items;
    }
    ...
    if (menuId === MenuIDs.CONTACT_DETAIL && selectedContact) {
      ...
      if (!contact) return [{ id: 'not_found', label: 'Contact Not Found', type: 'toggle' as MenuItemType }];
      return [
        { id: 'c_name', label: `Name: ${contact.firstName} ${contact.lastName}`, type: 'toggle' as MenuItemType },
        ...
        {
          id: 'edit_contact', label: 'Edit Contact', type: 'action' as MenuItemType,
          action: () => { 
            setContactForm({
        ...
        {
          id: 'delete_contact', label: 'Delete Contact', type: 'action' as MenuItemType,
          action: () => { deleteContact(selectedContact); goBack(); },
        },
      ];
    }
    ...
    if (menuId === MenuIDs.NOTES_LIST) {
      const items: MenuItem[] = [{
        id: 'add_note', label: 'Add Note', type: 'navigation' as const, targetMenuId: MenuIDs.NOTE_ADD,
        action: () => { setNoteForm({ title: '', content: '' }); setIsEditingNote(false); setSelectedNote(null); navigateTo(MenuIDs.NOTE_EDIT); }
      }];
      notes.forEach(n => {
        items.push({
          id: n.id, label: n.title, type: 'navigation' as const, targetMenuId: MenuIDs.NOTE_DETAIL, hasChevron: true,
          action: () => { setSelectedNote(n.id); navigateTo(MenuIDs.NOTE_DETAIL); }
        });
      });
      return items;
    }
    ...
    if (menuId === MenuIDs.NOTE_DETAIL && selectedNote) {
      ...
      if (!note) return [{ id: 'not_found', label: 'Note Not Found', type: 'toggle' as const }] as MenuItem[];
      const lines = note.content.split('\n');
      return [
        { id: 'note_title', label: `📝 ${note.title}`, type: 'toggle' as const },
        ...lines.map((line, i): MenuItem => ({ id: `line_${i}`, label: line || ' ', type: 'toggle' as const })),
        {
          id: 'edit_note', label: 'Edit Note', type: 'action' as const,
          action: () => { setNoteForm({ title: note.title, content: note.content }); setIsEditingNote(true); navigateTo(MenuIDs.NOTE_EDIT); },
        },
        {
          id: 'delete_note', label: 'Delete Note', type: 'action' as const,
          action: () => { deleteNote(selectedNote); goBack(); },
        },
      ] as MenuItem[];
    }
    ...
    navState.currentMenuId, playlists, selectedPlaylistId, sortedSongs, navigateTo, 
    addingToPlaylistId, playlistSearchQuery, handlePlaylistSearchSelect,
    selectedGenreId, globalSearchResults, handleSearchSelect, favorites, 
    sortedArtists, selectedArtistId,
    librarySongs, libraryArtists,
    notes, selectedNote, getNote, deleteNote, goBack,
    contacts, selectedContact, getContact, deleteContact,
    chassisColor, clockSettings, backlightTimeout, setBacklightTimeout,
    selectedAlbumId, fetchLibrary, renamePlaylist, deletePlaylist, music
  ]);
  ...
  }, [isBooting, navState.currentMenuId, navState.selectedIndex, currentMenuItems, executeItemAction, handlePlayPause, setClockSettings]);

  const handleScroll = useCallback((direction: 'cw' | 'ccw') => {
    if (isBooting) return;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((tagName === 'input' || tagName === 'textarea') && !['ArrowUp', 'ArrowDown', 'Enter'].includes(e.key)) return;
      if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', ' ', 'Enter', 'Backspace', 'Escape'].includes(e.key)) {
        e.preventDefault();
      }
      switch (e.key) {
        case 'ArrowDown': case 'ArrowRight': handleScroll('cw'); break;
        case 'ArrowUp': case 'ArrowLeft': handleScroll('ccw'); break;
        case 'Enter': handleMenuSelect(); break;
        case 'Backspace': case 'Escape': case 'm': case 'M': handleBack(); break;
  ...
  if (menuId === MenuIDs.NOW_PLAYING && music.currentTrack) {
    ...
    ScreenComponent = <ClockScreen onExit={goBack} settings={clockSettings} isPlaying={music.isPlaying} hasActiveTrack={!!music.currentTrack} />;
  } else if (menuId === MenuIDs.CONTACT_EDIT) {
    ScreenComponent = (
      <div className="w-full h-full bg-white flex flex-col">
        <StatusBar title={isEditingContact ? 'Edit Contact' : 'New Contact'} isPlaying={music.isPlaying} hasActiveTrack={!!music.currentTrack} theme="light" />
        <div className="flex-1 flex flex-col gap-2 p-4">
          <input className="border p-1 rounded text-sm text-black" placeholder="First Name" value={contactForm.firstName} onChange={e => setContactForm(s => ({ ...s, firstName: e.target.value }))} />
          <input className="border p-1 rounded text-sm text-black" placeholder="Last Name" value={contactForm.lastName} onChange={e => setContactForm(s => ({ ...s, lastName: e.target.value }))} />
          <input className="border p-1 rounded text-sm text-black" placeholder="Phone" value={contactForm.phone} onChange={e => setContactForm(s => ({ ...s, phone: e.target.value }))} />
          <input className="border p-1 rounded text-sm text-black" placeholder="Email" value={contactForm.email} onChange={e => setContactForm(s => ({ ...s, email: e.target.value }))} />
          <div className="flex gap-2 mt-2">
            <button className="bg-gray-300 text-black p-1 rounded flex-1" onClick={goBack}>Cancel</button>
            <button className="bg-blue-500 text-white p-1 rounded flex-1" onClick={() => {
              if (contactForm.firstName) {
                if (isEditingContact && selectedContact) updateContact(selectedContact, contactForm);
    ...
    ScreenComponent = (
      <div className="w-full h-full bg-white flex flex-col">
        <StatusBar title={isEditingNote ? 'Edit Note' : 'New Note'} isPlaying={music.isPlaying} hasActiveTrack={!!music.currentTrack} theme="light" />
        <div className="flex-1 flex flex-col gap-2 p-4">
          <input className="border p-1 rounded text-sm text-black" placeholder="Title" value={noteForm.title} onChange={e => setNoteForm(s => ({ ...s, title: e.target.value }))} />
          <textarea className="border p-1 rounded text-sm text-black flex-1 resize-none" placeholder="Content" value={noteForm.content} onChange={e => setNoteForm(s => ({ ...s, content: e.target.value }))} />
          <div className="flex gap-2 mt-2">
            <button className="bg-gray-300 text-black p-1 rounded flex-1" onClick={goBack}>Cancel</button>
            <button className="bg-blue-500 text-white p-1 rounded flex-1" onClick={() => {
              if (noteForm.title) {
                if (isEditingNote && selectedNote) updateNote(selectedNote, { title: noteForm.title, content: noteForm.content });
    ...
    ScreenComponent = (
      <div className="w-full h-full bg-white flex flex-col">
        <StatusBar title="Set Location" isPlaying={music.isPlaying} hasActiveTrack={!!music.currentTrack} theme="light" />
        <div className="flex-1 flex flex-col justify-center p-4">
          <input className="border p-2 rounded w-full text-black mb-4" placeholder="City Name" value={locationInput}
            onChange={e => setLocationInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') { setClockSettings(s => ({ ...s, location: locationInput })); goBack(); } }}
            autoFocus
          />
          <div className="flex gap-2">
            <button onClick={goBack} className="bg-gray-300 text-black p-2 rounded flex-1">Cancel</button>
            <button onClick={() => { setClockSettings(s => ({ ...s, location: locationInput })); goBack(); }} className="bg-blue-500 text-white p-2 rounded flex-1">Save</button>
          </div>
        </div>
      </div>
    );
    ...
    ScreenComponent = (
      <div className="w-full h-full bg-white flex flex-col">
        <StatusBar title="Search Playlist" isPlaying={music.isPlaying} hasActiveTrack={!!music.currentTrack} theme="light" />
        <div className="flex-1 flex flex-col overflow-hidden w-full">
          ...
          <div className="flex-1 overflow-y-auto ipod-scrollbar w-full">
            <MenuScreen
              title=""
              items={currentMenuItems}
              selectedIndex={navState.selectedIndex}
              onItemClick={(idx) => { selectIndex(idx); const item = currentMenuItems[idx]; if (item) executeItemAction(item); }}
              isPlaying={music.isPlaying}
              hasActiveTrack={!!music.currentTrack}
              hideTitle={true}
            />
    ...
    ScreenComponent = (
      <MenuScreen
        title={
          menuId === MenuIDs.HOME ? "Karan's iPod" :
            menuId === MenuIDs.SETTINGS ? 'Settings' :
              menuId === MenuIDs.EXTRAS ? 'Extras' :
                menuId === MenuIDs.MUSIC ? 'Music' :
                  menuId === MenuIDs.BACKLIGHT_SETTINGS ? 'Backlight Timer' :
                    menuId.toLowerCase().replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
        }
        items={currentMenuItems}
        selectedIndex={navState.selectedIndex}
        onItemClick={(idx) => { selectIndex(idx); const item = currentMenuItems[idx]; if (item) executeItemAction(item); }}
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4">
      <div style={{ width: 358 * scale, height: 700 * scale, position: 'relative', flexShrink: 0 }}>
        <div className="relative bg-[#e3e3e3] rounded-[30px] shadow-2xl overflow-hidden border-4 border-[#d1d1d1]"
          style={{
            width: '358px', height: '700px',
            ...
            boxShadow: 'inset 4px 4px 24px #FFFFFF, inset 0px 0px 32px rgba(233, 230, 224, 0.5), inset 8px -56px 100px rgba(46, 45, 45, 0.6)'
          }}
        >
          <div className="absolute overflow-hidden" style={{ top: '24px', left: '12px', width: '310px', height: '339px', border: '8px solid #000000', borderRadius: '5px', background: '#000', boxSizing: 'content-box' }}>
            <div className="w-full h-full relative bg-white flex flex-col text-black">
              {isBooting ? <BootScreen /> : ScreenComponent}
              {!isBooting && <div className="absolute inset-0 pointer-events-none z-50" style={{ background: 'conic-gradient(from 227.11deg at 67.66% 38.91%, #FFFFFF 0deg, rgba(255, 255, 255, 0) 360deg)', backgroundBlendMode: 'lighten', opacity: 0.1, borderRadius: '4px' }} />}
              {isDimmed && <div className="absolute inset-0 z-60 pointer-events-none" style={{ background: 'rgba(0,0,0,0.7)', transition: 'opacity 0.5s ease', borderRadius: '4px' }} />}
            </div>
          </div>
          <div className="absolute" style={{ top: '407px', left: 'calc(50% - 125.46px + 0.46px)', width: '250.92px', height: '250.92px' }}>
            <ClickWheel
              onScroll={handleScroll} onMenu={handleBack} onSelect={handleMenuSelect}
