...
export const useSettings = () => {
    ...
    const toggleFavorite = (trackData: FavoriteTrack | string) => {
        setFavorites(prev => {
            ...
            if (isFavorited) {
                ...
                const newTrack = typeof trackData === 'string'
    ...
    useEffect(() => {
        ...
        const fetchWeather = async () => {
            ...
            try {
                ...
                if (loc.includes('london')) { lat = 51.5074; lon = -0.1278; }
                else if (loc.includes('new york')) { lat = 40.7128; lon = -74.0060; }
                else if (loc.includes('tokyo')) { lat = 35.6762; lon = 139.6503; }
                else if (loc.includes('san francisco')) { lat = 37.7749; lon = -122.4194; }
                else if (loc.includes('paris')) { lat = 48.8566; lon = 2.3522; }
    ...
    // Helper to map WMO codes to text/icons
